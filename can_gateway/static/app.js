/* CAN Gateway add-on — scan-only panel */
let lastStatus = {};
let modulesCache = [];
...
function renderStatus() {
  ...
  $("conn-status").innerHTML = `
    ...
    <tr><td>Ostatni skan</td><td>${st.last_scan_status || "—"} ${formatTime(st.last_scan_at)}</td></tr>`;
  updateKeyBanner(st);
  $("footer-status").textContent = `${connectionLabel(st)} | moduły: ${modulesCache.length}`;
}
...
function renderModules() {
  ...
  for (const m of modulesCache) {
    ...
    tr.innerHTML = `
      ...
      <td>${m.name || "—"}</td>
      <td>${m.mac || "—"}</td>
      <td>${formatTime(lastStatus.last_scan_at)}</td>`;
    tb.appendChild(tr);
  }
}
...
async function loadAll() {
  ...
  if (!res.ok) {
    $("scan-status").textContent = `Błąd API: HTTP ${res.status}`;
    return;
  }
...
async function runScan() {
  ...
  try {
    ...
    if (!res.ok || !data.ok) {
      ...
      $("scan-status").textContent = `Skan OK: ${data.module_count ?? "?"} moduł(ów)${partial}`;
      if (Array.isArray(data.warnings) && data.warnings.length) {
    ...
    $("scan-status").textContent = `Błąd: ${err}`;
  } finally {
